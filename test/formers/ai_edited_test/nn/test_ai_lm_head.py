...
class TestLMHead(unittest.TestCase):
    ...
    def test_lm_head_forward_normal(self):
        ...
        with patch("paddlefleet.nn.lm_head.calc_lm_head_logits", return_value=paddle.randn([2, 8, 100])) as mock_fn:
            head(x)
    ...
    def test_lm_head_sharded_state_dict_multi_gpu(self):
        ...
        config = self._make_config(tensor_model_parallel_size=2)
        head = LMHead(config)
        with patch("paddlefleet.nn.lm_head.build_sharded_state_dict") as mock_build:
