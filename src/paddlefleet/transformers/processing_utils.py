...
class _LazyAutoProcessorMapping(dict):
    ...
    _MAPPING_NAMES = {
        "image_processor": ("paddlefleet.transformers.auto.image_processing", "AutoImageProcessor"),
        "video_processor": ("paddlefleet.transformers.auto.video_processing", "AutoVideoProcessor"),
        "feature_extractor": ("paddlefleet.transformers.auto.feature_extraction", "AutoFeatureExtractor"),
        "tokenizer": ("paddlefleet.transformers.auto.tokenizer", "AutoTokenizer"),
    }
        ...
        for lookup_location in lookup_locations:
            for custom_class in lookup_location._extra_content.values():
                if isinstance(custom_class, tuple):
                    ...
                    return custom_class
        raise ValueError(f"Could not find module {module_name} in `paddlefleet`.")
