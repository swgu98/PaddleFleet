...
本文通过使用一组 Function Call 数据集和[Qwen/Qwen3-0.6B](https://huggingface.co/Qwen/Qwen3-0.6B)模型演示如何使用[PaddleFleet](https://github.com/PaddlePaddle/PaddleFleet)训练一个支持 Function Call 能力的模型。

本次实验旨在说明如何基于 PaddleFleet 训练一个支持 Function Calling 能力的模型，验证模型 Function Calling 能力的性能和效果。整个实验使用1张40G A100显卡进行训练，耗时约5分钟。
...
```shell
paddlefleet-cli train function_call.yaml
```
