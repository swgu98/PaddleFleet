...
本文以 [Qwen/Qwen3-0.6B](https://huggingface.co/Qwen/Qwen3-0.6B) 模型为例，演示如何基于 [PaddleFleet](https://github.com/PaddlePaddle/PaddleFleet) 先进行 SFT 微调，再进行 DPO 训练，构建一个指令遵循能力更强、回答质量更高的对齐模型。

本次实验旨在说明如何在 PaddleFleet 框架下完成 SFT 与 DPO 的串联训练，并验证模型在指令理解与偏好对齐方面的效果提升。整个实验使用1张40G A100显卡进行训练，耗时约10分钟。
...
logging_dir: ./vdl/paddlefleet_qwen3_0p6b_sft_emoji/
output_dir: ./checkpoints/paddlefleet_qwen3_0p6b_sft_ckpts_emoji/
disable_tqdm: true
...
```shell
paddlefleet-cli train sft_full_emoji.yaml
```
...
### model
model_name_or_path: ./checkpoints/paddlefleet_qwen3_0p6b_sft_ckpts_emoji/
_attn_implementation: flashmask
...
logging_steps: 1
gradient_accumulation_steps: 16
logging_dir: ./vdl/paddlefleet_qwen3_0p6b_dpo_emoji/
output_dir: ./checkpoints/paddlefleet_qwen3_0p6b_dpo_ckpts_emoji/
disable_tqdm: true
eval_accumulation_steps: 16
...
```shell
paddlefleet-cli train dpo_full_emoji.yaml
```
