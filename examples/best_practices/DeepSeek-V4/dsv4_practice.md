...
DeepSeek-V4 引入了 CSA+HCA 混合注意力、mHC 残差连接、Muon 优化器等全新架构，我们在 PaddleFleet 中实现了对 DeepSeek-V4 系列模型训练的全面支持，涵盖 PP（流水线并行）、EP（专家并行）、CP（上下文并行）、DeepEP 专家通信、Muon 优化器、BF16 混合精度、重计算、RoPE fusion、高性能 FP8 MoE 融合模块、 fused mHC kernel 以及 flex checkpoint 等训练优化能力。
...
可按如下方式安装支持版本：
pip install paddlefleet==1.2.0.post0
...
paddlefleet-cli train dsv4_sft_4K.yaml
```

如果需要训练开启 PaddleFleet 高效训练所需的融合 kernel 开关，需要显式修改 `DeepSeek-V4-Flash-Base/config.json`，添加以下字段：
