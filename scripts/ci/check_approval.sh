...
PADDLEFLEET_TRAINER_APPROVERS="From00"
PADDLEFLEET_TRAINER_FILES=(
    "paddlefleet/trainer/training_args.py"
    "paddlefleet/cli/hparams/"
)
for FILE in "${PADDLEFLEET_TRAINER_FILES[@]}"; do
    ...
    if [ "${HAS_MODIFIED}" != "" ] && [ "${PR_ID}" != "" ]; then
        ...
        APPROVER_LIST=(${PADDLEFLEET_TRAINER_APPROVERS})
        check_approval 1 "${APPROVER_LIST[@]}"
    fi
done

PADDLEFLEET_REQUIREMENTS_APPROVERS="zjjlivein From00"
PADDLEFLEET_REQUIREMENTS_FILES=(
    "requirements.txt"
)
for FILE in "${PADDLEFLEET_REQUIREMENTS_FILES[@]}"; do
    HAS_MODIFIED=$(git diff --name-only "${DIFF_BASE}" HEAD -- | grep "^${FILE}" || true)
    if [ "${HAS_MODIFIED}" != "" ] && [ "${PR_ID}" != "" ]; then
        echo_line="You must be approved by zjjlivein or From00 for changes in ${FILE}.\n"
        APPROVER_LIST=(${PADDLEFLEET_REQUIREMENTS_APPROVERS})
        check_approval 1 "${APPROVER_LIST[@]}"
    fi
done
