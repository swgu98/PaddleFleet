...
这是一个用于验证 PaddleFleet 模型训练 loss 值的自动化测试框架。
...
```bash
# 运行所有 XPU CI 测试
cd /paddle/sjx_cuda12.6_py310/formers_test/PaddleFleet
...
================================================================================
Command: paddlefleet-cli train tests/xpu_ci/config/ernie_21b_sft.yaml
Working Directory: /paddle/sjx_cuda12.6_py310/formers_test/PaddleFleet
...
================================================================================
Executed Command: paddlefleet-cli train tests/xpu_ci/config/ernie_21b_sft.yaml
Working Directory: /paddle/sjx_cuda12.6_py310/formers_test/PaddleFleet
Config File: tests/xpu_ci/config/ernie_21b_sft.yaml
Baseline File: tests/xpu_ci/base_value/ernie_21b_sft_loss.json
Tolerance: 1e-06
...
这样做的好处：
- ✅ 命令一目了然，便于理解和调试
- ✅ 支持任意 shell 命令（不限于 paddlefleet-cli）
...
2. **编写测试函数**
   ...
   def test_your_model(project_root, base_value_dir, log_file):
       ...
       """
       # 直接写你要执行的 shell 命令
       cmd = "paddlefleet-cli train tests/xpu_ci/config/your_config.yaml"
...
3. **生成基准值**
   ...
   # 手动运行你的命令
   cd /paddle/sjx_cuda12.6_py310/formers_test/PaddleFleet
   paddlefleet-cli train tests/xpu_ci/config/your_config.yaml | tee output.log
...
**示例 1：标准 paddlefleet-cli 命令**
```python
def test_llama_training(project_root, base_value_dir, log_file):
    cmd = "paddlefleet-cli train tests/xpu_ci/config/llama_7b.yaml"
...
def test_with_env_vars(project_root, base_value_dir, log_file):
    cmd = "CUDA_VISIBLE_DEVICES=0,1,2,3 XPU_VISIBLE_DEVICES=0,1 paddlefleet-cli train config.yaml"
...
def test_with_preprocessing(project_root, base_value_dir, log_file):
    ...
    paddlefleet-cli train tests/xpu_ci/config/model.yaml --data /tmp/processed
    """
